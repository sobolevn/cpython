...
class Example:
    ...
    output.  `Example` defines the following attributes:
      ...
      - exc_group: The exception group message generated by the example
        (including all sub exceptions' details), if
        the example is expected to generate an exception; or `None` if
        it is not expected to generate an exception.  This exception
        message is compared against the return value of
        `traceback.format_exception_only()`.  `exc_msg` ends with a
    ...
    def __init__(self, source, want,
        ...
        if exc_msg is not None and not exc_msg.endswith('\n'):
            exc_msg += '\n'
        if exc_group is not None and not exc_group.endswith('\n'):
            exc_group += '\n'
        # Store properties.
        self.source = source
        self.want = want
...
class DocTestParser:
    ...
    # NOTE: we parse exception groups with `_ExceptionGroupParser`.
    ...
    def _parse_example(self, m, name, lineno):
        ...
        if m := self._EXCEPTION_RE.match(want):
            ...
            exc_msg = None
            # Exception is not set, but maybe exception group is expected?
            exc_group = self._try_parse_exception_group(want)
    ...
    def _try_parse_exception_group(self, text_to_parse):
        """
        ...
        """
        parsed = _ExceptionGroupParser(text_to_parse).parse()
...
class _ExceptionGroupParser:
    """Parses fancy exception group tracebacks from doctests."""

    # Some elements are static, the simplest (and probably fastest) way
    # is to just compare them as strings.
    ...
    class _Fail(Exception):
        """Raised when parser cannot parse the input."""
    ...
    def parse(self):
        ...
        Returns a list of strings formatted as `format_exception_only` does.
        Or `None` if the exception group cannot be parsed from the input.

        Each item we parse consists of:
        - exception group info
        - (optional) nested tracebacks / `...`
    ...
    def _visit_traceback(self, *, is_exception_group=False):
        ...
        # Generally, we don't care about it, unless it is a SyntaxError.
        ...
        while (iteration := next(counter)):
            ...
            if current.startswith(self._indent(self._eg_prefix)):
                replaced = current.replace(self._eg_prefix, '').strip()
    ...
    def _visit_message(self):
        ...
        while (iteration := next(counter)):
            ...
            if not current.startswith(self._indent(self._eg_prefix)):
                if iteration == 1:
                    # This does not look like it is a proper exception group,
                    # we need to have at least one exception message.
                    self._finish()
                return
    ...
    def _visit_syntax_error(self):
        while True:
            current = self._current_line
            if current.startswith(self._indent(self._eg_traceback_prefix)):
                self._add_result(current, is_syntax_eror=True)
                self._go_to_next_line()
                continue
            return
    ...
    def _finish(self, *, success=False):
        if success:
            raise self._Done
        raise self._Fail(self._text_lines)

    def _add_result(self, line, *, is_syntax_eror=False):
        line = line.replace(self._eg_prefix, '', 1)
        if is_syntax_eror:
            # SyntaxError format is special, only add a single space.
            line = f' {line}'
        else:
    ...
    def _indent(self, other):
        return f"{'  ' * self._indent_level}{other}"
    ...
    def __run(self, test, compileflags, out):
        ...
        for examplenum, example in enumerate(test.examples):
            ...
            # The example raised an exception:  check if it was expected.
                ...
                # raise ValueError(example.exc_group, exc_msg, example.exc_group == exc_msg)

                # If `example.exc_msg` and `example.exc_group` are both None,
                # then we weren't expecting an exception.
                if example.exc_msg is None and example.exc_group is None:
                    outcome = BOOM
                # We expected an exception:  see whether it matches.
                elif (
                    example.exc_msg
                    and check(example.exc_msg, exc_msg, self.optionflags)
                ):
                    outcome = SUCCESS
                elif (
                    example.exc_group
                    and check(example.exc_group, exc_msg, self.optionflags)
                ):
                    outcome = SUCCESS
                elif (
                    # Another chance if they didn't care about the detail.
                    example.exc_msg
                    and self.optionflags & IGNORE_EXCEPTION_DETAIL
                ):
