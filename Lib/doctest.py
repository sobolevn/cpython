...
class Example:
    ...
    output.  `Example` defines the following attributes:
      ...
      - exc_group: The tuple of exception group messages generated by the example,
        the example is expected to generate an exception group; or `None` if
        it is not expected to generate an exception.  This is compared against
        the return value of `traceback.format_exception_only()`.
    ...
    def __eq__(self, other):
        ...
        return self.source == other.source and \
               ...
               self.exc_group == other.exc_group

    def __hash__(self):
        return hash((self.source, self.want, self.lineno, self.indent,
                     self.exc_msg, self.exc_group))
...
class DocTestParser:
    ...
    # A regular expression for handling `want` strings that contain
    # expected exception groups.  It divides `want` into three pieces:
    #    - the exception group traceback stack (`stack`)
    #    - the exception group message (`msg`), as generated by
    #      traceback.format_exception_only()
    #    - the nested exceptions / exception groups (`nested`)
    # `msg` may have multiple lines.  We assume/require that the
    # exception message is the first non-indented line starting with a word
    # character following the traceback header line.
    _EXCEPTION_GROUP_RE = re.compile(r"""
        ^\s+\+\ Exception\ Group\ Traceback  # header, left part
        \s+\( most\ recent\ call\ last \):   # header, right part with brackets
        \s*$                                 # toss trailing whitespace on the header
        (?P<stack> .*?)                      # don't blink: absorb stuff until...
        # a line *starts* with ` | \w` and ends with `(N sub-exceptions)`
        ^\s+\|\s(?P<msg>\w+ .*? \( \d+ \s sub-exceptions \) )$
        # next, parse all nexted exceptions and groups,
    ...
    def parse(self, string, name='<string>'):
        ...
        for m in self._EXAMPLE_RE.finditer(string):
            ...
            (source, options, want, exc_msg, exc_group) = \
                     self._parse_example(m, name, lineno)
            # Create an Example, and add it to the list.
            if not self._IS_BLANK_OR_COMMENT(source):
                output.append(Example(source, want, exc_msg, exc_group,
                                    lineno=lineno,
    ...
    def _parse_example(self, m, name, lineno):
        ...
        if m := self._EXCEPTION_GROUP_RE.match(want):
            exc_group = self._parse_exception_group(want, m)
        else:
            exc_group = None
        ...
        return source, options, want, exc_msg, exc_group

    # Regular expression used to replace `|` in exception group traceback
    # while maintaining the nesting level
    _EXCEPTION_GROUP_LSTRIP = re.compile(
        r'^(\s+)\|\s',
        re.MULTILINE,
    )

    # Split lines based on `+---- ... ----` patterns
    _EXCEPTION_GROUP_LINE_SUB = re.compile(
        r'\+-[\+-]+(?:\s(?:\d|\.\.\.)\s)?-+',
    )

    def _parse_exception_group(self, want, matched):
        """
        Parse exception group to an intermediate format.

        It is the same one used by `format_exception_only`,
        so it would be easy to compare the two.

        Returns a tuple of strings that identify exceptions' messages.
        """
        # At this point `nested` looks something like:
        # +-+---------------- 1 ----------------
        ...
        nested = matched.group('nested')
        # First, remove `|` chars from the left side, but keep the spacing:
        nested = self._EXCEPTION_GROUP_LSTRIP.sub(r'\1', nested)
        # Next, remove all separating lines by splitting the text
        # nested = self._EXCEPTION_GROUP_LINE_SUB.split(nested)
        # Trim starting newlines and leading spaces, filter empty leftovers:
        # nested = [
        #     modified
        #     for line in nested
        #     if (modified := line.lstrip('\n').rstrip(' '))
        # ]
        raise ValueError(nested)

        indent = len(nested[0]) - len(nested[0].lstrip(' '))
        return (
            matched.group('msg'),
            *self._process_nested_exceptions(''.join(nested), indent),
        )

    def _process_nested_exceptions(self, nested, indent):
        raise ValueError(nested)
    ...
    def __run(self, test, compileflags, out):
        ...
        for examplenum, example in enumerate(test.examples):
            ...
            # The example raised an exception:  check if it was expected.
                ...
                formatted_ex = traceback.format_exception_only(*exception[:2], show_group=True)
